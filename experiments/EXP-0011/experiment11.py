# ============================================================
# Q-ALPHA | EXP-0011 | experiment11.py
# Polygon.io | $3K account | Dynamic position sizing
# Fixes: runner position counting, active_tickers, pool sizing
# ============================================================
import os
import modal
import warnings
...
def run_simulation(test_df, price_pivot, threshold, label=""):
    ...
    test_df["signal"] = (test_df["prob"].values >= threshold).astype(int)
    ...
    for current_date in all_dates:
        ...
        available_slots = MAX_POSITIONS - len(significant_positions)
        if available_slots <= 0:
            deployed = (sum(p.deployed_capital() for p in active) + sum(p.deployed_capital() for p in runners))
            equity_curve.append({"date": current_date, "equity": pool + deployed})
            continue
        ...
        if (current_date in signal_lookup and
            ...
            for _, row in candidates.iterrows():
                ...
                if day_trade_count >= MAX_TRADES_DAY:
                    break
                if len(significant_positions) >= MAX_POSITIONS:
                    break
...
def main():
    print("="*55)
    print("  Q-ALPHA | EXP-0011 | $3K Dynamic Sizing Pipeline")
    print("  Running on Modal cloud")
    print("="*55)

    result = run_exp011.remote()
