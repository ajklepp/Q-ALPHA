# ============================================================
# Q-ALPHA | EXP-0015 | experiment15.py
# EXP-0014 + 5 gap-day features | next-day-open entry | VIX regime
# Filter BEFORE labeling: gap>=3% AND vol_ratio>=2x
# 19 features (14 pre-gap + 5 gap-day) | BracketPosition from EXP-0012
# ============================================================
import os
import modal
import warnings
...
def calculate_option_d_labels(df):
    """
    Option D for candidate rows: entry at next-day Open, stop=entry-1xATR,
    target=entry+2xATR. Forward simulation on entry day through +4 (5 days total).
    ...
    for i in range(len(df) - 7):
        ...
        label = 0
        for j in range(entry_idx, min(entry_idx + 5, len(df))):
...
def process_stock(ticker, start, end, api_key):
    ...
    try:
        ...
        # F15-F19 gap-day features — known at gap-day close, entry is next-day open
...
def run_simulation(test_df, price_pivot, open_pivot, vix_series,
                   label="", trade_universe=None):
    """Simulate bracket trades; entry at next-day open after gap-day signal."""
    import pandas as pd
    import numpy as np
    ...
    for current_date in all_dates:
        ...
        signal_date = pd.Timestamp(current_date) - BDay(1)
        if (signal_date in day_lookup and
            ...
            if entry_date not in open_pivot.index:
                ...
                for _, row in candidates.iterrows():
                    ...
                    try:
                        entry_px = float(open_pivot.loc[entry_date, ticker])
                    except Exception:
                        continue
...
def evaluate_gates(base_rate, mq, bt, wf_pass_count, wf_total):
    """Score EXP-0015 success gates; return pass count and status."""
    gates = {
        "base_rate_35_45": BASE_RATE_MIN <= base_rate <= BASE_RATE_MAX,
        "precision_42": mq["precision"] >= PRECISION_GATE,
        "lift_1_15": mq["lift"] >= LIFT_GATE,
        "sharpe_1_10": bt["sharpe"] >= SHARPE_GATE,
        "max_dd_12": bt["max_dd"] >= MAX_DD_GATE,
        "wf_3_4": wf_pass_count >= WF_PASS_MIN,
        "trades_40": bt["n_trades"] > 40,
        "beats_exp0014": bt["sharpe"] > EXP0014_SHARPE,
    }
...
def run_exp015():
    ...
    for i, ticker in enumerate(download_universe):
        ...
        if raw is None:
            continue
        total_rows += len(raw)
        price_slice = raw[["Open", "Close", "ticker"]].copy()
    ...
    print(f"\n  Feature Importance (all {len(FEATURE_COLUMNS)}):")
    gap_day_feats = {
    ...
    for rank, (feat, score) in enumerate(imp, 1):
        marker = " *" if feat in gap_day_feats else ""
        print(f"    {rank:>2}. {feat:<22} {score:.0f}{marker}")
    if gap_in_top5:
        print(f"  Gap-day features in top 5: {sorted(gap_in_top5)}")
    else:
        print("  Gap-day features in top 5: none")
    ...
    # ── STEP 4: Price pivots (Close for marks, Open for entry) ─
    print("\nSTEP 4: Building price pivots...")
    ...
    # ── STEP 5: Simulation ───────────────────────────────────
    print("\nSTEP 5: Running simulation (entry at next-day Open)...")
    bt = run_simulation(
        test_df, price_pivot, open_pivot, vix_series,
        label="OOS 2023-2024",
        trade_universe=TRADE_UNIVERSE,
    )
    if bt is None:
    ...
    for ts, te, vs, ve, label in wf_windows:
        ...
        pp_w = price_slice.pivot_table(
            ...
            values="Close", aggfunc="last")
        op_w = price_slice.pivot_table(
            index=price_slice.index, columns="ticker",
            values="Open", aggfunc="last")

        res = run_simulation(
            test_w, pp_w, op_w, vix_series,
    ...
    print(f"  vs EXP-0014 Sharpe : {bt['sharpe']:.2f} vs {EXP0014_SHARPE}")
    ...
    print(f"    EXP-0012 Sharpe : {EXP0012_SHARPE:.2f}")
    print(f"    EXP-0013 Sharpe : 0.99")
    print(f"    EXP-0014 Sharpe : {EXP0014_SHARPE:.2f}")
    print(f"    EXP-0015 Sharpe : {bt['sharpe']:.2f}")
...
def main():
    print("=" * 55)
    print("  Q-ALPHA | EXP-0015 | Gap-Day Features + VIX Regime")
    print("  Running on Modal cloud")
    print("=" * 55)

    result = run_exp015.remote()
