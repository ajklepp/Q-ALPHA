# ============================================================
# Q-ALPHA | EXP-0016 | experiment16.py
# EXP-0015 + Day 0 Close entry (MOC) | 300-ticker universe
# 19 features | VIX regime | Option D label days +1 to +5
# ============================================================
import os
import modal
import warnings
...
def evaluate_gates(base_rate, mq, bt, wf_pass_count, wf_total):
    """Score EXP-0016 success gates; return pass count and status."""
    gates = {
        "base_rate_35_45": BASE_RATE_MIN <= base_rate <= BASE_RATE_MAX,
        "precision_42": mq["precision"] >= PRECISION_GATE,
        ...
        "trades_80": bt["n_trades"] > TRADES_GATE,
        "beats_exp0015": bt["sharpe"] > EXP0015_SHARPE,
    }
...
def run_exp016():
    ...
    t0 = time.time()
    print("=" * 55)
    print("EXP-0016: Day 0 Close Entry | 300-Ticker Universe | VIX Regime")
    ...
    print(f"Features : {len(FEATURE_COLUMNS)} (14 pre-gap + 5 gap-day)")
    ...
    print(f"Expected base rate range: 35-45%")
    if labels.mean() < 0.20:
    ...
    gap_day_feats = {
        "close_vs_range_d0", "volume_ratio_d0", "upper_wick_ratio_d0",
        "gap_pct_d0", "body_ratio_d0",
    }
    top5_names = {f for f, _ in imp[:5]}
    gap_in_top5 = gap_day_feats & top5_names
    print(f"\n  Feature Importance (top 10):")
    for i, (feat, score) in enumerate(imp[:10]):
        marker = " [gap-day]" if feat in gap_day_feats else ""
        print(f"  #{i + 1}: {feat} = {score:.4f}{marker}")
    print("KEY CHECK: Are gap-day features (F15-F19) in top 5?")
    if gap_in_top5:
        print(f"  YES — {sorted(gap_in_top5)}")
    else:
        print("  NO")
    ...
    # ── STEP 4: Price pivot (Close for entry + marks) ─────────
    ...
    # ── STEP 5: Simulation ───────────────────────────────────
    print("\nSTEP 5: Running simulation (entry at Day 0 Close / MOC)...")
    bt = run_simulation(
    ...
    test_preds = (test_df["prob"] >= THRESHOLD_BULL).astype(int)
    test_precision = precision_score(
        test_df["target"], test_preds, zero_division=0)
    test_base_rate = float(test_df["target"].mean())
    test_lift = (test_precision / test_base_rate
                 if test_base_rate > 0 else 0.0)
    print(f"\n  Test set diagnostics:")
    print(f"Test precision @ threshold: {test_precision:.1%}")
    ...
    print(f"  Base Rate          : {base_rate:.2%}  (gate: 35-45%)")
    ...
    print(f"  vs EXP-0015 Sharpe : {bt['sharpe']:.2f} vs {EXP0015_SHARPE}")
    ...
    print(f"\n  Experiment Progression (Sharpe):")
    print(f"    EXP-0012: {EXP0012_SHARPE:.2f}  EXP-0013: 0.99  "
          f"EXP-0014: {EXP0014_SHARPE:.2f}  EXP-0015: {EXP0015_SHARPE:.2f}  "
          f"EXP-0016: {bt['sharpe']:.2f}")
...
def main():
    print("=" * 55)
    print("  Q-ALPHA | EXP-0016 | Day 0 Close + 300-Ticker Universe")
    print("  Running on Modal cloud")
    print("=" * 55)

    result = run_exp016.remote()
    ...
    print(f"  Gap in top 5  : {result.get('gap_in_top5', False)}")
