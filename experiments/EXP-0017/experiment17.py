# ============================================================
# Q-ALPHA | EXP-0017 | experiment17.py
# EXP-0016 + Rules ScoreCard (replaces LightGBM)
# 6-rule gap-day candle scorecard | 300-ticker universe
# ============================================================
import os
import modal
import warnings
...
MIN_SCORE              = 5
SCORECARD_MAX          = 11
SCORECARD_THRESHOLD    = MIN_SCORE / SCORECARD_MAX
...
def run_simulation(test_df, price_pivot, vix_series,
    ...
    for current_date in all_dates:
        ...
        threshold_today = SCORECARD_THRESHOLD
...
def evaluate_gates(base_rate, mq, bt, wf_pass_count, wf_total):
    """Score EXP-0017 success gates; return pass count and status."""
    gates = {
        "base_rate_18_28": BASE_RATE_MIN <= base_rate <= BASE_RATE_MAX,
        "scorecard_precision_25": mq["precision"] >= PRECISION_GATE,
        "lift_1_10": mq["lift"] >= LIFT_GATE,
        "sharpe_1_50": bt["sharpe"] >= SHARPE_GATE,
        "max_dd_10": bt["max_dd"] >= MAX_DD_GATE,
        "wf_3_4": wf_pass_count >= WF_PASS_MIN,
        "trades_60": bt["n_trades"] > TRADES_GATE,
        "score_monotonic": mq.get("monotonic", False),
    }
...
def run_exp017():
    ...
    t0 = time.time()
    print("=" * 55)
    print("EXP-0017: Rules ScoreCard | 300-Ticker Universe | MOC Entry")
    print(f"Universe : {len(TRADE_UNIVERSE)} stocks + SPY (regime)")
    print(f"Filter   : gap>={GAP_MIN:.0%} AND vol_ratio>={VOL_RATIO_MIN:.0f}x")
    print(f"Entry    : Day 0 Close (MOC order)")
    print(f"Selector : 6-rule ScoreCard (min {MIN_SCORE}/{SCORECARD_MAX} pts)")
    print(f"Threshold: {SCORECARD_THRESHOLD:.4f} (normalized score)")
    print(f"Validate : {VALIDATE_START} to {VALIDATE_END}")
    print(f"Test     : {SIM_START} to {SIM_END}")
    print("=" * 55)

    # ── STEP 1: Download OHLCV + VIX ─────────────────────────
    ...
    print(f"Base rate: {labels.mean():.1%}")
    print(f"Positive samples: {int(labels.sum())}")
    print(f"Total candidate rows: {len(labels)}")
    print(f"Expected base rate range: 18-28%")
    if labels.mean() < 0.20:
        print("WARNING: Base rate below 20% — check entry price logic")
    if labels.mean() > 0.50:
        print("WARNING: Base rate above 50% — check for look-ahead bias")
    ...
    if vix_series.empty:
        print("  WARNING: VIX series empty — defaulting to 20.0 all days")

    # ── STEP 2: Score candidates with rules scorecard ────────
    print("\nSTEP 2: Scoring candidates with rules scorecard...")
    scorecard = ScoreCard()
    ...
    print(f"Validate: {len(validate_df):,}  Test: {len(test_df):,}")

    val_raw = scorecard.score_series(validate_df)
    val_norm = val_raw / SCORECARD_MAX
    print_scorecard_diagnostics(
        val_norm, val_raw, validate_df["target"], label="2022 Validation")
    mq_val = evaluate_scorecard(
        validate_df["target"], val_raw, val_norm, label="2022 Validation")

    test_df = add_scorecard_scores(test_df, scorecard)
    test_raw = test_df["score_pts"]
    test_norm = test_df["prob"]
    print_scorecard_diagnostics(
        test_norm, test_raw, test_df["target"], label="OOS 2023-2024")
    mq_test = evaluate_scorecard(
        test_df["target"], test_raw, test_norm, label="OOS 2023-2024")

    # ── STEP 3: Price pivot (Close for entry + marks) ─────────
    print("\nSTEP 3: Building price pivot...")
    ...
    # ── STEP 4: Simulation ───────────────────────────────────
    print("\nSTEP 4: Running simulation (entry at Day 0 Close / MOC)...")
    bt = run_simulation(
    ...
    test_precision = mq_test["precision"]
    test_base_rate = mq_test["base_rate"]
    test_lift = mq_test["lift"]
    print(f"\n  Test set summary:")
    print(f"Test scorecard precision: {test_precision:.1%}")
    print(f"Test base rate: {test_base_rate:.1%}")
    print(f"Lift: {test_lift:.2f}x")
    print(f"Trades generated: {bt['n_trades']}")

    # ── STEP 5: Walk-forward ─────────────────────────────────
    print("\nSTEP 5: Walk forward across 4 windows...")
    wf_windows = [
    ...
    for ts, te, vs, ve, label in wf_windows:
        ...
        test_w = add_scorecard_scores(test_w, scorecard)
        ...
        res = run_simulation(
            test_w, pp_w, vix_series,
            label=label, trade_universe=TRADE_UNIVERSE)
        if res is None:
            continue
    ...
    print(f"\n{'=' * 55}")
    print("EXP-0017 RESULTS")
    print(f"{'=' * 55}")
    print(f"  Runtime            : {time.time() - t0:.0f}s")
    print(f"  Universe size      : {len(TRADE_UNIVERSE)} tickers")
    print(f"  Selector           : Rules ScoreCard (min {MIN_SCORE} pts)")
    ...
    print(f"  Max Drawdown       : {bt['max_dd']:.2%}  (gate: <10%)")
    print(f"  Total Trades       : {bt['n_trades']}  (gate: >{TRADES_GATE})")
    print(f"  Win Rate           : {bt['win_rate']:.2%}")
    print(f"  Base Rate          : {base_rate:.2%}  (gate: 18-28%)")
    print(f"  Scorecard precision: {mq_val['precision']:.2%}  (gate: >25%)")
    print(f"  Test precision     : {test_precision:.2%}")
    print(f"  Lift (validation)  : {mq_val['lift']:.2f}x")
    print(f"  Monotonicity       : {'PASS' if mq_val.get('monotonic') else 'FAIL'}")
    print(f"  Walk Forward       : {wf_pass_count}/{len(wf_results)}")

    print(f"\n  VIX Regime Breakdown:")
    for regime in ("LOW_VOL", "NORMAL", "ELEVATED", "EXTREME"):
        tc = bt["vix_trade_counts"].get(regime, 0)
        pnl = bt["vix_pnl"].get(regime, 0.0)
        print(f"    {regime:<10} trades={tc:<4}  PnL=${pnl:,.2f}")

    print(f"\n  Experiment Progression:")
    print(f"    EXP-0012 Sharpe: {EXP0012_SHARPE:.2f}  EXP-0014: {EXP0014_SHARPE:.2f}  "
          f"EXP-0015: {EXP0015_SHARPE:.2f}  EXP-0016: {EXP0016_SHARPE:.2f}  "
          f"EXP-0017: {bt['sharpe']:.2f}")
    ...
    return {
        ...
        "test_lift": round(float(test_lift), 4),
        "universe_size": len(TRADE_UNIVERSE),
        "monotonic": mq_val.get("monotonic", False),
        "selector": "Rules",
        "vix_trade_counts": bt["vix_trade_counts"],
        "vix_pnl": bt["vix_pnl"],
...
def main():
    print("=" * 55)
    print("  Q-ALPHA | EXP-0017 | Rules ScoreCard + 300-Ticker Universe")
    print("  Running on Modal cloud")
    print("=" * 55)

    result = run_exp017.remote()
    ...
    print(f"  Walk Fwd      : {result['wf_count']}/{result.get('wf_total', 4)}")
    print(f"  Universe      : {result.get('universe_size', '?')} tickers")
    print(f"  Monotonic     : {result.get('monotonic', False)}")
    print(f"  Selector      : {result.get('selector', 'Rules')}")
