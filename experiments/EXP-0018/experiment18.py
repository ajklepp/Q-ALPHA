# ============================================================
# Q-ALPHA | EXP-0018 | experiment18.py
# Phase-3 ablation: catalyst filter ONLY (gap>=3% + vol>=2x)
# NO LightGBM, NO rules scorecard — bracket + regime sizing only
# Sacred: BracketPosition, classify_profile, get_regime (from EXP-0012)
# Entry/label: Day-0 Close (MOC) + ATR_d0 (Phase-2 cleanest alignment)
# Universe: same dynamic screener as EXP-0016/0017 (~300 tickers)
# ============================================================
import os
import modal
import warnings
...
MC_SIMS              = 5000
MC_P_GATE            = 0.05
SELECTOR             = "NONE"  # every catalyst day is a candidate
ENTRY_CONVENTION     = "MOC_CLOSE_ATR_D0"  # label + sim aligned
S4_RUNNER_DAYS       = 30
GAP_MIN              = 0.03
VOL_RATIO_MIN        = 2.0
SHARPE_GATE          = 1.50
...
def run_simulation(test_df, price_pivot, vix_series,
    ...
    SELECTOR=NONE: every catalyst-filtered row may trade subject only to
    regime slot caps, MAX_TRADES_DAY, and pool sizing (no ML/rules gate).
    """
    import pandas as pd
    import numpy as np
    ...
    for current_date in all_dates:
        ...
        # SELECTOR=NONE — no probability threshold
        ...
        if (signal_date in day_lookup and
            ...
            # SELECTOR=NONE: no ML/rules filter. Deterministic capacity tiebreak only.
...
def evaluate_gates(base_rate, bt, wf_pass_count, wf_total, mc_p):
    """EXP-0018 ablation gates (no model precision). Report ALL; FAIL loudly."""
    gates = {
    ...
    gates_meta = {
        "base_rate_18_28_info": BASE_RATE_MIN <= base_rate <= BASE_RATE_MAX,
        "trades_gt_60_info": bt["n_trades"] > TRADES_GATE,
    }
    ...
    if passed == len(gates):
        ...
        status = "FAIL"
    return gates, gates_meta, passed, status
...
def run_monte_carlo(all_closed, n_sims=MC_SIMS, seed=42):
    ...
    if len(real_rets) < 5 or real_rets.std() <= 0:
        return {"ok": False, "p_value": None, "pct": None, "real_sharpe": None}
...
def run_exp018():
    """Catalyst-filter-only ablation (no ML, no rules scorecard)."""
    import pandas as pd
    import numpy as np
    import warnings
    import time
    ...
    t0 = time.time()
    print("=" * 55)
    print("EXP-0018: Catalyst-Only Ablation | NO Selector | MOC Entry")
    ...
    print("Label    : Option D (2R before stop within 5 days) — same entry")
    print(f"Selector : {SELECTOR} (capacity tiebreak = ticker A-Z only)")
    print(f"Validate : {VALIDATE_START} to {VALIDATE_END} (base-rate window)")
    ...
    print("\n  === Trade count + base-rate transparency (Precision@0.60 = N/A) ===")
    ...
    print(f"  Validate rows: {len(validate_df):,}  Test rows: {len(test_df):,}")

    vix_series = get_vix_data(api_key, DATA_START, DATA_END, spy_df=spy_ohlc)

    print("\nSTEP 2: Building price pivot (Close = MOC entry + marks)...")
    ...
    print("\nSTEP 3: Simulation (catalyst-only, SELECTOR=NONE)...")
    bt = run_simulation(
        test_df,
        price_pivot,
        vix_series,
    ...
    print("\nSTEP 4: Walk forward across 4 windows...")
    wf_windows = [
    ...
    print(f"\nSTEP 5: Monte Carlo ({MC_SIMS} simulations)...")
    mc = run_monte_carlo(bt["all_closed"], n_sims=MC_SIMS)
    if mc["ok"]:
        print(f"  Real Sharpe  : {mc['real_sharpe']:.3f}")
        print(f"  Beats random : {mc['pct']:.1f}%")
        print(f"  p-value      : {mc['p_value']:.3f}")
        print(f"  Result       : {'PASS' if mc['pass'] else 'FAIL'}")
    else:
        print("  FAIL: not enough trades for Monte Carlo")

    gates, gates_meta, gates_passed, verdict = evaluate_gates(
        base_rate, bt, wf_pass_count, len(wf_results), mc.get("p_value")
    )

    print(f"\n{'=' * 55}")
    print("EXP-0018 RESULTS")
    print(f"  Verdict: {verdict} ({gates_passed}/{len(gates)} gates)")
    for name, ok in gates.items():
        print(f"    {'PASS' if ok else 'FAIL'} {name}")
    if verdict != "PASS":
        print("  *** FAIL / INVESTIGATE — do not promote to /candidates ***")
    print(f"{'=' * 55}")
