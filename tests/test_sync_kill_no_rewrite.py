...
Restored from 4269bca after PR #22 / structure-stop land reintroduced the
stopPrice-only read (live ATRC 0.0->53.13 rewrite loop).
"""
...
class TestSyncKillTarget5313(unittest.TestCase):
    ...
    def test_stop_price_zero_aux_at_target_skips_place_order(self):
        order = _FakeOrder(43126, qty=2, aux_price=53.13, stop_price=0.0)
        ib = _FakeIB(
            trades=[_FakeTrade(order)],
            positions=[_FakePos("ATRC", 2)],
        )
        buf = io.StringIO()
        with patch("sys.stdout", buf):
            result = tsd_exit.sync_kill_quantity(ib, _atrc_leg(kill_price=53.13), "ATRC")
        self.assertTrue(result)
        self.assertEqual(ib.cancelled, [])
        self.assertEqual(ib.placed, [])
        self.assertIn("kill already working @53.13", buf.getvalue())
        self.assertIn("skip rewrite", buf.getvalue())

    def test_aux_51_96_target_53_13_ratchets_once(self):
        order = _FakeOrder(43126, qty=2, aux_price=51.96, stop_price=0.0)
        ib = _FakeIB(
            trades=[_FakeTrade(order)],
            positions=[_FakePos("ATRC", 2)],
        ...
        self.assertEqual(ib.cancelled, [43126])
        self.assertEqual(len(ib.placed), 1)
        self.assertAlmostEqual(float(ib.placed[0].auxPrice), 53.13)

        buf = io.StringIO()
        with patch("sys.stdout", buf):
            tsd_exit.sync_kill_quantity(ib, _atrc_leg(kill_price=53.13, kill_oid=ib.placed[0].orderId), "ATRC")
        self.assertEqual(len(ib.placed), 1)
        self.assertIn("skip rewrite", buf.getvalue())

    def test_same_stop_price_and_aux_price_skips(self):
        order = _FakeOrder(43126, qty=2, aux_price=53.13, stop_price=53.13)
        ib = _FakeIB(
            trades=[_FakeTrade(order)],
            positions=[_FakePos("ATRC", 2)],
        )
